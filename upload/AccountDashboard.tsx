...
export function AccountDashboard({ user, orders, wishlist, addresses: initialAddresses }: AccountDashboardProps) {
  ...
  const [2faDialogOpen, set2faDialogOpen] = useState(false);
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => set2faDialogOpen(true)} className="rounded-full border border-[#C9A96E] px-4 py-1.5 text-xs font-medium text-[#C9A96E] transition hover:bg-[#C9A96E] hover:text-[#0F0F0F]">Enable 2FA</button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {/* 2FA Dialog */}
      <Dialog.Root open={2faDialogOpen} onOpenChange={set2faDialogOpen}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" />
          <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-background p-6 shadow-xl text-center">
            ...
            <div className="flex gap-3">
              <button onClick={() => set2faDialogOpen(false)} className="flex-1 rounded-full border border-border py-2.5 text-sm font-medium">Cancel</button>
              <button onClick={() => set2faDialogOpen(false)} className="flex-1 rounded-full bg-[#C9A96E] py-2.5 text-sm font-semibold text-[#0F0F0F]">Confirm</button>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
